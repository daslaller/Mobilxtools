...
:: Read file bytes using PowerShell - increased to 16 bytes for longer signatures
...
for /f "tokens=1,2 delims==" %%i in ('set') do (
    if "%%i" neq "header" if "%%i" neq "file" if "%%i" neq "filename" if "%%i" neq "found" if "%%i" neq "processedfiles" if "%%i" neq "totalfiles" if "%%i" neq "progress" if "%%i" neq "signatureCount" if "%%i" neq "totalSignatures" if "%%i" neq "scantype" if "%%i" neq "recursive" if "%%i" neq "inputdir" if "%%i" neq "outputdir" if "%%i" neq "logfile" (
...
echo Signature Progress: [!progressbar!] (%~1%%) %~2
echo.
goto :eof